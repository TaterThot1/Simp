...
client.on('guildMemberRemove', member => {
  ...
  const log = member.guild.channels.cache.find(ch => ch.name === MainLogName);
  if (!channel) return;
...
client.on("guildBanAdd", function (guild, user) {
  const channel = guild.channels.cache.find(ch => ch.name === WelcomeChannel);
  const log = guild.channels.cache.find(ch => ch.name === BanLogName);
  if (!channel) return;
