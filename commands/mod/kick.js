...
module.exports = class KickCommand extends Command {
    ...
    run(message, client) {
       ...
       const log = message.guild.channels.cache.find(ch => ch.name === BanLogName);
       const Discord = require('discord.js');
       ...
       const mod = message.member.user;
        ...
        if (!message.member.hasPermission("kickMembers")) {
			return message.reply('You dont have the correct permissions to use this command');
		}
		return tuser.kick(reason)
			.then(() => log.send({embed: {color: '#FF0000', title: 'Kick Log', description: `${user} has been kicked by ${mod}.`}}));
	}
};
